...
Strategy selection in automated research calls for more than maximizing a single in-sample metric. Common practice combines Sharpe ratio, drawdown, win rate, and turnover into a composite score \citep{prado_2018}. Recent work has proposed scores that penalize validation decay and regime instability \citep{harvey_2015, prado_2018}, and Yu et al.~\citep{yu_2023_synergistic_alpha} evaluate formulaic alphas on several efficiency-aware axes rather than by return alone.
...
Table 1 summarizes the two strategies analyzed in detail. Table 2 reports how the search evolved across seasons. The full feature catalog is provided in Supplementary Table~S1, and the inter-season design decisions are documented in Supplementary Table~S2.
...
\emph{Note: Annual returns are computed from the deployment simulation with independent yearly resets (EUR~500 per year) and modeled costs of 0.20 \% fee + 0.10 \% slippage. Sharpe ratios are computed from daily equity changes on active trading days (days with at least one closed position) and annualised with $\sqrt{365}$; calendar-time Sharpe, which includes all days, is the more conservative headline metric and is reported in Section~4.6. The 2026 figure covers 1 January--31 March and was used in the deployment-selection criterion; it is not an independent prospective test and is based on only four trades.}
...
\emph{Note: S12 iter 5502 returns are from the deployment simulation with modeled costs (0.20 \% fee + 0.10 \% slippage) and independent yearly resets (EUR~500 per year). BTC/USDC buy-and-hold returns are computed from Binance daily close prices and ignore funding and custody costs. Max drawdown for buy-and-hold is the largest peak-to-trough decline over 2021--2025.}
...
All Sharpe ratios reported by the engine and in this section are computed from daily equity changes and annualised with $\sqrt{365}$, the standard convention for cryptocurrency markets. We compute this ratio in two ways. The \textbf{active-day Sharpe} uses only days with at least one closed position, discarding inactive days; it is the metric the backtest engine optimizes and is reported in Tables~1 and~3 to match the engine output. The \textbf{calendar-time Sharpe} uses the full daily equity curve, including all zero-return inactive days; it is the more conservative headline metric because it reflects the experience of a continuously invested account. The calendar-time values are lower than the active-day values (see Table~1) and should be treated as the primary descriptive evidence for risk-adjusted performance.
