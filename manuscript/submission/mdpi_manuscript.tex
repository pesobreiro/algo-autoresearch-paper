...
\Author{Pedro Sobreiro $^{1,4,*}$\orcidA{}, Domingos Martinho $^{2,3}$\orcidB{}, Pedro Ramos $^{2,4}$\orcidC{}, Antonio Pratas $^{2}$\orcidD{}}
...
Most published frameworks still rely on heavy human supervision or treat the LLM as a black-box oracle that proposes strategies without explicit safeguards against leakage or overfitting \citep{bailey_lopez_prado_2014_deflated_sharpe_ratio, kaufman_2012_leakage_data_mining, sobreiro_2026_bitcoin_market_prediction}. At the same time, the emerging generation of autonomous quant-agent systems and their evaluation environments has focused on capability—richer agent architectures and verifiable task benchmarks \citep{frai_2025_llm_equity_review, zhang_2024_finagent_kdd}—while recent studies show a gap between simulated evaluation and deployable trading performance \citep{liu_2021_finrl_simulation_gap, sobreiro_2026_bitcoin_market_prediction}.
...
\funding{No external funding was received for this research. Domingos Martinho acknowledges support from NECE --- Research Centre for Business Sciences (UIDB/04630/2020).}
