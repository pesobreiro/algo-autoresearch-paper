...
\Author{Pedro Sobreiro $^{1,4,*}$\orcidA{}, Domingos Martinho $^{2,3}$\orcidB{}, Pedro Ramos $^{2,4}$\orcidC{}}
\AuthorNames{Pedro Sobreiro, Domingos Martinho, Pedro Ramos}
...
Most published frameworks still rely on heavy human supervision or treat the LLM as a black-box oracle that proposes strategies without explicit safeguards against leakage or overfitting. At the same time, the emerging generation of autonomous quant-agent systems and their evaluation environments has focused on \emph{capability}---richer agent architectures and verifiable task benchmarks---while post-mortems of highly automated pipelines increasingly document catastrophic divergence between validation and live performance \citep{chen_2025_redqueen}. Recent work argues that statistical rigor must be enforced \emph{structurally} within the discovery loop rather than checked after the fact \citep{benjamini_hochberg_1995}. The field is therefore building more powerful proposal engines faster than the safeguards that would make their output trustworthy.

We propose a \textbf{season-structured research architecture}. In our framework, an experimental \emph{season} is the operational unit of a human-in-the-loop interaction cycle for quantitative investment, grounded in mixed-initiative governance principles \citep{horvitz_1999, amershi_2019} and generative curriculum learning \citep{bengio_2009}. Inside a season, an LLM agent autonomously proposes, codes, validates, and backtests directional cryptocurrency strategies under fixed constraints. At the season boundary, a human reviewer inspects the distribution of accepted candidates, updates the objective, tightens acceptance gates, and resets the scoring weights---typically 15--30 minutes per transition. This boundary is more than a logistical checkpoint: it is a mixed-initiative governance mechanism that constrains the scope of autonomous decisions and guards against objective drift. The sequence of seasons also implements a curriculum. Earlier seasons use looser gates to explore the strategy space, while later seasons impose stricter temporal-integrity and holdout criteria, ending in a cross-asset replication that tests whether the pipeline generalizes beyond the asset on which it was developed. By confining unbounded autonomous search to bounded, governed seasons, the design makes leakage prevention and overfitting control enforceable as first-class architectural invariants.

The LLM in this architecture is a \textbf{high-volume proposal generator}, not a source of superior financial intuition. The pipeline's value lies in the validation, scoring, and governance layers that turn a large stream of machine-generated proposals into a small set of testable candidates and reject those that would not survive out-of-sample scrutiny. Our ablation study (Section~4.8) finds no consistent statistical evidence that the LLM selects feature combinations that significantly outperform random subsets drawn from the same catalog. The scientific contribution is therefore the validation and governance architecture, not any consistent superiority of the LLM at discovering features.

The cross-season design has inferential limits. Because the human reviewer updates season objectives, acceptance gates, and scoring weights after observing prior holdout results, the sequence of seasons is an \textbf{adaptive meta-search}---a prolonged model-selection procedure rather than a single pre-registered protocol---that gradually consumes out-of-sample information. The reported passive-holdout results are therefore interpreted as \emph{descriptive evidence that the architecture can reject unsound candidates}, not as \emph{independent prospective proof} that the surviving strategies possess durable trading edge. We return to this limitation in Sections~4.6, 4.7, and 5.2, and we calibrate our empirical claims accordingly throughout the paper.

This paper proposes and evaluates a season-structured prescriptive-analytics architecture that uses an LLM to autonomously generate directional cryptocurrency strategies, with leakage prevention and overfitting control built in as core design constraints. It treats trading as a closed-loop analytics problem: predictive modeling, simulation, and optimization are embedded in a workflow \citep{shmueli_2011, bertsimas_2020} that prescribes actionable decisions subject to temporal-integrity and statistical constraints.

The paper combines three elements. Methodologically, we integrate automated proposal generation (here driven by an LLM used as a high-volume candidate generator), leakage-free feature constraints, layered validation, and an evolving composite score into a single autonomous research loop. These safeguards are not new in isolation, but their combination inside a season-structured, human-in-the-loop pipeline is novel, to our knowledge. On the engineering side, the implementation relies on open-source choices---a local 7B-parameter LLM, AST-based future-leak detection, Numba-accelerated backtesting, and a temperature-driven curriculum---that make thousands of autonomous iterations feasible on consumer hardware. Empirically, we evaluate two candidates, one on BNB/USDT and one on BTC/USDC, that passed the pipeline's gates and showed positive passive-holdout performance under conservative calendar-time metrics. These trading results illustrate that the architecture can select candidates worth further study; they do not imply that any specific strategy will generalize to future markets.
...
Predictive analytics forecasts future outcomes from historical data, while prescriptive analytics recommends or automates decisions that optimise an objective under constraints \citep{davenport_2007, shmueli_2011}. Bertsimas and Kallus \citep{bertsimas_2020} formalise the shift from prediction to prescription, showing how machine-learning predictions can be combined with optimisation to produce actionable policies. Separately, a strand of \emph{autonomous analytics pipelines} has appeared in quantitative finance, in which agents iteratively propose, backtest, and refine strategies: FinAgent \citep{zhang_2024_finagent}, for example, formalises a self-improving loop for quantitative investment. This has prompted verifiable evaluation environments and closed-loop benchmarks for such agents \citep{sun_wang_an_2023}, which shows the growing recognition that autonomous quant systems must be judged on reproducible, cost-aware, out-of-sample protocols rather than single headline backtests.

Our contribution is an \textbf{offline prescriptive-analytics research pipeline}: it discovers, validates, and selects decision policies, but it does not trade live. Deploying a selected policy would require a separate online system that reads current market features and prescribes positions. We treat the offline research phase as prescriptive analytics because its output is a complete decision policy optimised end-to-end for a stated objective and validated on holdout data; but we do not claim live deployment results. This aligns with the design-science view that a research artefact can be evaluated by its ability to produce well-validated prescriptions even before operational deployment \citep{hevner_2004}. Relative to the benchmark-driven agent literature \citep{sun_wang_an_2023}, our emphasis is complementary: we do not focus on richer agents or broader task suites; instead, we focus on a season-structured governance layer that decides which autonomously generated candidates are trustworthy enough to retain.
...
Our pipeline puts these principles into practice by confining full autonomy to the inside of a season and keeping human review for season transitions. The human reviewer updates the season objective, acceptance gates, and scoring weights based on the distribution of accepted strategies. This is a mixed-initiative design: the machine runs thousands of experiments and surfaces patterns, while the human steers the research direction and guards against objective drift. The season-transition review is therefore more than a logistical checkpoint: it is a governance mechanism that constrains the scope and goals of autonomous experimentation.
...
\item
  \textbf{Optimisation}: Optuna tunes stop-loss, take-profit, and probability threshold on the training/validation window.
\item
...
where the weights \(w_i\) were adjusted across seasons. In Seasons 9--12, the weights emphasised holdout Sharpe and drawdown control; strategies with negative holdout Sharpe or validation-holdout decay were heavily penalised. Inside the Bayesian optimiser, the objective function changed from maximising the minimum validation Sharpe across sub-windows (score mode) to maximising the average holdout return (profit mode), with hard constraints on drawdown and trade count.
...
The selected candidate's passive holdout Sharpe (2.87 on active trading days) comes from a population of 212 accepted strategies in Season 11, so it is also subject to selection bias. Under the simplifying assumption that the 212 accepted holdout Sharpes are i.i.d.~normal with the observed mean (1.37) and standard deviation (0.61), the expected maximum among 212 draws is approximately 3.06 (95 \% interval: 2.66--3.62), and the observed maximum active-day holdout Sharpe of 2.95 lies within the range expected from pure selection; however, the positive mean of the accepted strategies (1.37) indicates that the accepted population as a whole has positive holdout performance. This selection-bias calculation is related to the robust Sharpe inference developed by [20] and [23].
...
Table 1 summarises the two strategies analysed in detail. Table 2 reports how the search evolved across seasons. The full feature catalog is provided in Supplementary Table~S1, and the inter-season design decisions are documented in Supplementary Table~S2.
...
Total return & 58.2 \% (2022--2026, simple sum) & 27.7 \% (2021--2025) \\
...
\emph{Note: S11 metrics are produced by \texttt{deployment/evaluate\_models.py} with independent yearly backtests (initial capital EUR~500, \texttt{max\_pos=5}, ATR kill 3.0). The holdout year (2025) was never used during hyper-parameter optimisation; the Jan--Mar 2026 period was used to select the production model from 30 re-evaluated candidates and is therefore a noisy post-selection observation (four trades), not an independent prospective test. Validation and holdout Sharpe ratios labelled `active-day' are computed from daily equity changes on days with at least one closed position and annualised with $\sqrt{365}$; the `Calendar-time holdout Sharpe' is computed from the full daily equity curve including inactive days and is the more conservative headline metric.}

The experiments were run on a local workstation with a consumer GPU serving the Qwen2.5-7B-Instruct model via llama.cpp. We did not log exact wall-clock time per iteration, but the full study (34\,333 iterations across twelve seasons) required about 6--8 weeks of continuous execution, including restarts after memory-related crashes in early seasons. The LLM inference and Optuna optimisation together consumed an estimated 500--800 kWh. Per-iteration latency varied from seconds (for strategies rejected at the code-validation layer) to several minutes (for strategies that ran the full Optuna loop). Future work will add instrumentation to record precise GPU hours and energy consumption.

\subsection{Reproducibility protocol}\label{reproducibility-protocol}

To enable independent replication, we fixed the software environment, random seeds, data sources, and execution flow described below. All code, experiment logs, and generated artefacts are available in the repository associated with this submission.

\textbf{Persistent archive.} All code, experiment logs, and generated artefacts will be deposited in a persistent public repository (e.g., Zenodo or an institutional data archive) upon manuscript acceptance, and the final DOI will be added to the Data Availability Statement. Until then, the materials are available from the corresponding author on reasonable request.

\textbf{Software environment.} The pipeline was executed under Python 3.13.11 with the following core package versions: pandas 3.0.1, NumPy 2.4.3, Numba 0.65.1, XGBoost 3.3.0, scikit-learn 1.9.0, Optuna 4.9.0, Joblib 1.5.3, PyArrow 12.0 or later, and llama.cpp serving Qwen2.5-7B-Instruct (quantised GGUF, Q4\_K\_M). The multi-timeframe feature-merge module is imported from \texttt{features.technical} (commit hash recorded in \texttt{requirements.txt}). The full dependency list is in \texttt{requirements.txt}. We recommend creating a dedicated virtual environment and installing the pinned versions; newer versions may alter random number generation or sampler behaviour and are not guaranteed to reproduce the exact numeric results reported here.

\textbf{Random seeds.} Determinism was enforced at three points: (i) the XGBoost classifier uses \texttt{random\_state=42}; (ii) Optuna's TPE sampler is initialised with \texttt{seed=42} and \texttt{n\_startup\_trials=20} for the SL/TP/threshold study (and \texttt{n\_startup\_trials=max(5,\ n\_trials\ //\ 4)} when XGBoost hyper-parameters are also tuned); (iii) scikit-learn's \texttt{TimeSeriesSplit} is used for walk-forward cross-validation. Because some operations rely on Numba JIT compilation and BLAS threading, byte-identical reproduction across operating systems is not guaranteed; the statistical conclusions, however, are robust to the residual non-determinism.
...
\textbf{Data.} OHLCV bars at 15-minute, 4-hour, and daily resolutions were downloaded from the Binance public API and stored as Parquet files in a dedicated data directory (\texttt{\textasciitilde{}/crypto\_data}). The raw data are publicly available; the ticker mappings are \texttt{bnb\_15m\_usdt\_binance}, \texttt{bnb\_4h\_usdt\_binance}, \texttt{bnb\_1d\_usdt\_binance} for BNB/USDT and \texttt{btc\_15m\_usdt\_binance}, etc., for BTC/USDC. We used the closing prices from the daily series to compute the buy-and-hold benchmarks. A \texttt{download\_data.py} helper script in the repository performs the exact Binance API calls used in this study and records the date ranges of each file.
...
\item
  \texttt{best\_models/season\_\{season\}/} --- accepted model artefacts.
\item
...
\textbf{Execution flow.} A single season is launched with \texttt{python\ main.py} after copying \texttt{config.yaml.example} to \texttt{config.yaml} and starting the local LLM server. The runner executes the loop described in Section~3.1: proposal, validation, training, Optuna optimisation, and holdout evaluation. The configuration file specifies the ticker, exchange, data directory, season identifier, and acceptance minima; the exact feature sets, indicator parameters, SL/TP/threshold ranges, and XGBoost hyper-parameters are written by the LLM into \texttt{pipeline/research\_params.py} each iteration. For post-hoc verification, the deployment scripts \texttt{deployment/evaluate\_models.py} and \texttt{deployment/backtest\_deploy.py} replay the selected iterations using the saved model and parameters, without calling the LLM.

\textbf{Reproduction checklist.} To replicate the two case studies reported in Section~4: (1) install the pinned environment; (2) download the Binance OHLCV data for BNB/USDT and BTC/USDC at the three timeframes; (3) start the LLM server with the Qwen2.5-7B-Instruct GGUF model; (4) run \texttt{python\ main.py} with the appropriate season number; (5) for the selected iterations, run the deployment scripts with the parameters recorded in \texttt{best\_models/season\_\{N\}/iter\_\{XXXX\}/meta.json}. Because the search is expensive, a partial reproduction can focus on rerunning the saved case studies, which is computationally cheap and fully deterministic given the saved model and parameters.
...
The strategy's AUC of 0.5721 shows only marginal predictive power at the individual-bar level; the economic value comes from combining a calibrated probability threshold, asymmetric risk management (7.8 \% stop-loss vs.~7.1 \% take-profit), and an ADX regime filter [34] that avoids weak-trend entries. The wider stop-loss relative to take-profit came from Optuna optimisation under the bear-market validation introduced in Season 10: it gives profitable trades room to survive short-term reversals while the high-probability threshold and ADX filter keep entry quality high enough that smaller, more frequent gains offset occasional larger losses.

\textbf{Benchmark comparison.} Table 5 compares the strategy's annual returns with a passive buy-and-hold investment in BNB/USDT.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1429}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1429}}@{}}
\caption{Annual performance of the selected production candidate (S11 iter 1077, BNB/USDT)}\\
\toprule\noalign{}
...
\emph{Note: Annual returns are computed from the deployment simulation with independent yearly resets (EUR~500 per year) and modelled costs of 0.20 \% fee + 0.10 \% slippage. Sharpe ratios are computed from daily equity changes on active trading days (days with at least one closed position) and annualised with $\sqrt{365}$; calendar-time Sharpe, which includes all days, is the more conservative headline metric and is reported in Section~4.6. The 2026 figure covers 1 January--31 March and was used in the deployment-selection criterion; it is not an independent prospective test and is based on only four trades.}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{Benchmark comparison: S11 iter 1077 vs.~BNB/USDT buy-and-hold}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedleft
Year
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\emph{Note: S11 iter 1077 returns are from the deployment simulation (\texttt{evaluate\_models.py}) with modelled costs 0.20 \% fee + 0.10 \% slippage. BNB/USDT buy-and-hold returns are computed from daily close prices downloaded from the Binance public API; they ignore funding, staking, and custody costs. The 2026 buy-and-hold figure covers 1 January--31 March 2026.}

\subsection{Cross-asset generalisation}\label{cross-asset-generalisation}

To test whether the design works across assets, we ran Season 12 on BTC/USDC with a training window starting in 2017. The selected candidate, S12 iter 5502, passed the validation gates and showed a validation Sharpe of 1.39 and a holdout Sharpe of 1.15, with a 27.7 \% holdout return and a maximum drawdown of -8.1 \% (Figure~4). Table~4 breaks down the annual performance. Returns are positive in every year; the weakest years were 2021 (a late-bull period) and 2024 (a sideways-to-down phase). The lower holdout Sharpe relative to S11 reflects Bitcoin's different volatility regime, but the candidate remained profitable and within risk limits.
...
\caption{Annual performance of the cross-asset replication (S12 iter 5502, BTC/USDC)}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Year
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\textbf{Benchmark comparison.} Table~8 compares the S12 strategy's annual returns with a passive buy-and-hold investment in BTC/USDC. As with the S11 case, the strategy underperforms buy-and-hold in strong bull years (2021, 2023, 2024) but avoids most of the bear-market drawdown and produced positive returns in 2022 and 2025 when the benchmark was negative.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{Benchmark comparison: S12 iter 5502 vs.~BTC/USDC buy-and-hold}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedleft
Year
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
2025 & 9.3 & -7.34 & +16.64 \\
\textbf{Total (2021--2025)} & \textbf{48.8} & \textbf{198.82} & \textbf{-150.02} \\
\textbf{Max drawdown} & \textbf{-8.0} & \textbf{-77.57} & \textbf{+69.57} \\
\end{longtable}

\emph{Note: S12 iter 5502 returns are from the deployment simulation with modelled costs (0.20 \% fee + 0.10 \% slippage) and independent yearly resets (EUR~500 per year). BTC/USDC buy-and-hold returns are computed from Binance daily close prices and ignore funding and custody costs. Max drawdown for buy-and-hold is the largest peak-to-trough decline over 2021--2025.}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig04_equity_s12_5502.png}
\caption{Equity curve --- S12 BTC/USDC iter 5502 (2021--2025).\label{fig4}}
\end{figure}
...
Figure 5 shows the top features for S12 iter 5502. Stochastic RSI and ADX at the daily and 15-minute timeframes dominate, followed by EMA differences and Bollinger Band width. The multi-timeframe pattern matches the findings of [30] for Bitcoin prediction: short-term momentum and medium-term trend strength carry complementary information. Reference [30] is by the same first author and shares the OHLCV data source and multi-timeframe feature-engineering approach; we cite it because it documents the same empirical regularity, not as independent validation.
...
Optuna parameter importance (Figure~6) shows that the probability threshold explains 82--88 \% of the optimisation variance across seasons, while stop-loss and take-profit contribute the remainder. This means that trade selection precision matters more than exact exit levels, once the exit range is broadly correct.
...
Figure~7 shows the sensitivity of S12 iter 5502 to round-trip transaction costs. Even with costs rising to 1.0 \%, the final equity remains well above the break-even line, so the strategy's edge is not an artefact of unrealistic cost assumptions.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1429}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1429}}@{}}
\caption{Exit-slippage stress-test}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedleft
Exit slippage (\%)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
\textbf{Trade counts by ADX regime.} Table 6 reports the trade counts underlying the regime means.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1333}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1333}}@{}}
\caption{Trade counts by ADX regime}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
ADX regime
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsection{Statistical rigour}\label{statistical-rigour}

All Sharpe ratios reported by the engine and in this section are computed from daily equity changes and annualised with $\sqrt{365}$, the standard convention for cryptocurrency markets. We compute this ratio in two ways. The \textbf{active-day Sharpe} uses only days with at least one closed position, discarding inactive days; it is the metric the backtest engine optimises and is reported in Tables~1 and~3 to match the engine output. The \textbf{calendar-time Sharpe} uses the full daily equity curve, including all zero-return inactive days; it is the more conservative headline metric because it reflects the experience of a continuously invested account. The calendar-time values are lower than the active-day values (see Table~1) and should be treated as the primary descriptive evidence for risk-adjusted performance.

Table~7 reports active-day Sharpe ratios, bootstrap p-values, approximate Deflated Sharpe Ratios (DSR), and percentile bootstrap 95~\% confidence intervals for the three main case studies. Calendar-time Sharpe values are reported in Table~1. These statistics are intended as \textbf{descriptive diagnostics} of the strategies' holdout behaviour, not as inferential guarantees or proof that the strategies will generalise. The bootstrap procedure tests the null hypothesis \(H_0: \mu \le 0\) against the one-sided alternative \(H_1: \mu > 0\), where \(\mu\) is the mean daily return on active trading days (days with at least one closed position). Both S11 models reject \(H_0\) at \(p < 0.0001\), and S12 rejects at \(p = 0.0012\). The approximate DSR values, which adjust for the raw number of trials in each season and for non-normality, remain positive: 11.78 for S11 iter 1077 and 3.24 for S12 iter 5502. These values should be interpreted as \textbf{optimistic upper bounds} because the DSR does not correct for the cross-season meta-search.

These statistics must be interpreted with their assumptions in mind. The bootstrap p-value relies on the sample of active trading days being representative of the strategy's future daily return distribution. This assumption is questionable for three reasons. First, the sample is conditional on days with at least one closed trade, so it omits inactive days and may overstate precision. Second, the number of active trading days is small (45--63), so the sampling distribution is noisy; the 95 \% bootstrap confidence intervals for the Sharpe ratio are wide (Table 7). Third, daily returns on active trading days are not independent: ACF(1) ranges from -0.24 to 0.18, and Jarque-Bera tests reject normality for the two S11 cases (p $\leq$ 0.0003). We therefore treat the p-values and DSR values as descriptive indicators rather than as exact frequentist probabilities or guarantees of future performance.

The approximate DSR is computed as follows. First, the variance of the Sharpe ratio under the null is adjusted for skewness and kurtosis following [26]:
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 18\tabcolsep) * \real{0.1026}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 18\tabcolsep) * \real{0.1026}}@{}}
\caption{Bootstrap p-values, approximate Deflated Sharpe Ratios, and 95~\% confidence intervals}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Case
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
...
The reported profitability in every calendar year (Tables~3 and~4) is based on a small sample of annual blocks (five years per strategy) and a low number of trades per year (four to eighty-two), and should not be mistaken for independent out-of-sample evidence. The LLM's feature proposals and the season acceptance gates were both informed by earlier seasons, so the design itself consumes out-of-sample information through an outer meta-search loop. The single-season bootstrap p-values and DSR values in Table~7 do not correct for this cross-season selection and are based on only 45--63 active trading days.
...
\item
  \textbf{Cross-season qualitative replication:} S12 (BTC/USDC) replicates the pipeline design after the lessons of S1--S11. This provides qualitative evidence of portability but is not a controlled ablation and does not rule out meta-overfitting.
\end{enumerate}

The evidence supports the narrower conclusion that the selected strategies are not trivial artefacts of in-sample curve fitting; it does not support unconditional claims about future performance.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1333}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 14\tabcolsep) * \real{0.1333}}@{}}
\caption{Temporal robustness of Sharpe ratios across window definitions}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Window type
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1600}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1600}}@{}}
\caption{Ablation study: random baselines versus LLM-selected configuration (S12 iter 5502)}\\
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Baseline
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
Fifth, all results are simulated with modelled costs. Entry trades include 0.10 \% slippage, but stop-loss and take-profit exits are filled at the triggered level without additional slippage; live fills may therefore differ. Sixth, the multi-timeframe feature-merge module is imported at runtime from an external package (\texttt{features.technical}), although an internal copy is provided for audit and reproducibility. Seventh, the ablation study (Section 4.8) is limited to one accepted model and a modest number of random feature subsets; it supports the interpretation that validation is the dominant contribution, but it does not constitute a full factorial decomposition of every pipeline component. Eighth, the cryptocurrency market is highly non-stationary, and past performance may not generalise to future regimes.
...
For quantitative researchers, the main implication is that LLMs can accelerate the strategy-proposal loop, but only if the surrounding infrastructure enforces temporal integrity and overfitting control. The LLM should not be trusted to avoid future leakage, to select hyper-parameters without validation, or to possess superior feature intuition; in our ablation, random feature subsets performed comparably. The value of the system lies in the validator-scorer-optimiser triad, not in the LLM alone.

For practitioners, the low-frequency, high-precision nature of the accepted candidates (tens to low-hundreds of trades over multi-year horizons) is attractive because it minimises transaction costs and operational risk. The regime filter, volatility kill-switch, and drawdown gates provide risk governance that is often missing in fully automated systems. However, the pipeline described here is an \textbf{offline research tool}; live deployment would require a separate online system with real-time data feeds, execution monitoring, position-sizing governance, and circuit breakers. We do not claim that the selected candidates are ready for live trading without that additional layer.

\subsection{Design principles}\label{design-principles}

Following the design-science paradigm in information-systems research \citep{hevner_2004}, the pipeline can be abstracted into three generalisable design principles that extend beyond the specific trading domain:
...
\authorcontributions{Conceptualisation, methodology, software, validation, formal analysis, investigation, data curation, writing --- original draft, writing --- review and editing, visualisation, supervision: Pedro Sobreiro. Methodology, writing --- review and editing: Domingos Martinho. All authors have read and agreed to the published version of the manuscript.}
...
\acknowledgments{During the preparation of this manuscript, the authors used a locally deployed large language model (Qwen2.5-7B-Instruct via llama.cpp) for code scaffolding, drafting suggestions, and scripted figure/table generation. The authors reviewed and edited all output and take full responsibility for the content of this publication. No proprietary, personal, or non-public data were sent to external APIs.}
...
\bibitem{athey_2019} Athey, S., \& Imbens, G. W. (2019). Machine learning methods that economists should know about. \emph{Annual Review of Economics, 11, 685--725}. \url{https://doi.org/10.1146/annurev-economics-080217-053433}.
...
\bibitem{kou_2024} Kou, Z.; Yu, H.; Luo, J.; Peng, J.; Li, X.; Liu, C.; Dai, J.; Chen, L.; Han, S.; Guo, Y. (2024). Automate strategy finding with LLM in quant investment. \emph{arXiv preprint arXiv:2409.06289}. \url{https://doi.org/10.48550/arXiv.2409.06289}.
...
\bibitem{mertens_2002} Mertens, E. (2002). Comments on the variance of the IID estimator in Lo (2002). Working paper, University of Basel. Available at \url{http://www.elmarmertens.com/research/discussion/soprano01.pdf}.
...
\bibitem{sun_wang_an_2023} Sun, S., Wang, R., \& An, B. (2023). Reinforcement learning for quantitative trading. \emph{ACM Transactions on Intelligent Systems and Technology, 14(3), 44:1--44:29}. \url{https://doi.org/10.1145/3582560}.
\bibitem{sutton_2018} Sutton, R. S., \& Barto, A. G. (2018). \emph{Reinforcement learning: An introduction (2nd ed.)}. MIT Press.

\bibitem{wang_2023} Wang, S.; Yuan, H.; Zhou, L.; Ni, L.M.; Shum, H.Y.; Guo, J. (2023). Alpha-GPT: Human-AI interactive alpha mining for quantitative investment. \emph{arXiv preprint arXiv:2308.00016}. \url{https://doi.org/10.48550/arXiv.2308.00016}.
...
\bibitem{deeparies_2025} Kim, J.; et al. (2025). DeepAries: Adaptive Rebalancing Interval Selection for Enhanced Portfolio Management. \emph{arXiv preprint arXiv:2510.14985}. \url{https://doi.org/10.48550/arXiv.2510.14985}.
